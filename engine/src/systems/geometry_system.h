...
typedef struct geometry_system_config {
    ...
    // there can and will be more then one of these per mesh.
...
/**
 * @brief Acquires an existing geometry id.
 * 
 * @param id The geometry indentifier to acquire by.
 ...
 * @return A pointer to the acquired geometry of nulltpr if failed.
 */
 ...
 * NOTE: vertex and index arrays are dynamically allocated and should be freed upon disposal.
 ...
 * @param tile_x The number of times the texture should tile across the x-axis. Must be non-zero.
 * @param tile_y The number of times the texture should tile across the y-axis. Must be non-zero.
 * @param name The name of the generated geometry.
 * @param material_name The name of the material to be used.
 * @return A geometry configuration which can be fed into geometry_system_acquire_from_config().
 */
