...
b8 resource_system_initialize(u64* memory_requirement, void* state, resource_system_config config) {
    if (config.max_loader_count == 0) {
        LFATAL("resource_system_initialize failed because config.max_loader_count==0.");
        return false;
    }
        ...
        for (u32 i = 0; i < count; ++i) {
            ...
            if (l->id != INVALID_ID) {
                if (l->type == loader.type) {
                    ...
                    LERROR("resource_system_register_loader - Loader of custom type %s already exists and will not be registered.", loader.custom_type);
                    return false;
                }
            }
        }
...
b8 resource_system_load_custom(const char* name, const char* custom_type, resource* out_resource) {
    ...
    out_resource->loader_id = INVALID_ID;
    LERROR("resource_system_load_custom - No loader for type %s was found.", custom_type);
    return false;
}
