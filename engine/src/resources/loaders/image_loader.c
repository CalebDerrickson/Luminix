...
b8 image_loader_load(struct resource_loader* self, const char* name, resource* out_resource)
{
    ...
    // TODO: Should be using an allocator here.
    image_resource_data* resource_data = lallocate(sizeof(image_resource_data), MEMORY_TAG_TEXTURE);
...
{
    if(!self || !resource) {
        LWARN("image_loader_unload called with nullptr for self or resource.");
        return;
    }
    ...
    if(resource->data) {
        lfree(resource->data, resource->data_size, MEMORY_TAG_TEXTURE);
        resource->data = 0;
