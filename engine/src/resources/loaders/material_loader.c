...
b8 material_loader_load(struct resource_loader* self, const char* name, resource* out_resource)
{
    ...
    string_format(full_file_path, format_str, resource_system_base_path(), self->type_path, name, ".lmt");
    ...
    if (!filesystem_open(full_file_path, FILE_MODE_READ, false, &f)) {
        LERROR("material_loader_load - unable to open material file for reading: '%s'.", full_file_path);
        return false;
    }
    ...
    string_ncopy(resource_data->name, name, MAX_MATERIAL_NAME_LENGTH);
    ...
    while (filesystem_read_line(&f, 511, &p, &line_length)) {
        ...
        if (strings_equali(trimmed_var_name, "version")) {
            ...
            string_ncopy(resource_data->name, trimmed_value, MAX_MATERIAL_NAME_LENGTH);
        } 
