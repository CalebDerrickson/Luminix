...
b8 upload_data_range(vulkan_context* context, VkCommandPool pool, VkFence fence, VkQueue queue, vulkan_buffer* buffer, u64* out_offset, u64 size, const void* data) {
    ...
    if (!vulkan_buffer_allocate(buffer, size, out_offset)) {
        LERROR("upload_data_range failed to allocate from the given buffer!");
        return false;
    }
...
// and make sure they exist. Validation layers should only be enabled on non-release builds.
    ...
    for (u32 i = 0; i < required_validation_layer_count; ++i) {
        ...
        if (!found) {
            LFATAL("Required validation layer is missing: %s", required_validation_layer_names[i]);
            return false;
        }
    }
    ...
    if (!vulkan_ui_shader_create(&context, &context.ui_shader)) {
        LERROR("Error loading built-in ui shader.");
        return false;
    }
...
b8 vulkan_renderer_backend_begin_frame(renderer_backend* backend, f32 delta_time) {
    ...
    VkResult result = vkWaitForFences(context.device.logical_device, 1, &context.in_flight_fences[context.current_frame], true, __UINT64_MAX__);
    if (!vulkan_result_is_success(result)) {
        LFATAL("In-flight fence wait failure! error: %s", vulkan_result_string(result, true));
        return false;
    }
    ...
    if (!vulkan_swapchain_acquire_next_image_index(
            ...
            0,
            &context.image_index)) {
        LERROR("Failed to acquire next image index, booting.");
        return false;
    }
...
b8 vulkan_renderer_backend_end_frame(renderer_backend* backend, f32 delta_time) {
    ...
    if (context.images_in_flight[context.image_index] != VK_NULL_HANDLE) {  // was frame
        VkResult result = vkWaitForFences(context.device.logical_device, 1, context.images_in_flight[context.image_index], true, __UINT64_MAX__);
        if (!vulkan_result_is_success(result)) {
            LFATAL("vkWaitForFences error: %s", vulkan_result_string(result, true));
        }
    }
...
b8 vulkan_renderer_begin_renderpass(struct renderer_backend* backend, u8 renderpass_id) {
    ...
    switch (renderpass_id) {
        ...
        case BUILTIN_RENDERPASS_UI:
            ...
            break;
        default:
            LERROR("vulkan_renderer_begin_renderpass called on unrecognized renderpass id: %#02x", renderpass_id);
            return false;
    }
...
b8 vulkan_renderer_end_renderpass(struct renderer_backend* backend, u8 renderpass_id) {
    ...
    switch (renderpass_id) {
        ...
        case BUILTIN_RENDERPASS_UI:
            ...
            break;
        default:
            LERROR("vulkan_renderer_end_renderpass called on unrecognized renderpass id:  %#02x", renderpass_id);
            return false;
    }
...
void create_command_buffers(renderer_backend* backend) {
    ...
    LDEBUG("Vulkan command buffers created.");
}
...
void vulkan_renderer_create_texture(const u8* pixels, texture* texture) {
    ...
    texture->internal_data = (vulkan_texture_data*)lallocate(sizeof(vulkan_texture_data), MEMORY_TAG_TEXTURE);
    ...
    if (!vulkan_result_is_success(VK_SUCCESS)) {
        LERROR("Error creating texture sampler: %s", vulkan_result_string(result, true));
        return;
    }
...
b8 vulkan_renderer_create_geometry(geometry* geometry, u32 vertex_size, u32 vertex_count, const void* vertices, u32 index_size, u32 index_count, const void* indices) {
    if (!vertex_count || !vertices) {
        LERROR("vulkan_renderer_create_geometry requires vertex data, and none was supplied. vertex_count=%d, vertices=%p", vertex_count, vertices);
        return false;
    }
    ...
    if (!internal_data) {
        LFATAL("vulkan_renderer_create_geometry failed to find a free index for a new geometry upload. Adjust config to allow for more.");
        return false;
    }
    ...
    if(!upload_data_range(
        ...
        total_size,
        vertices)) {
            LERROR("vulkan_renderer_create_geometry failed to upload vertex data to the buffer!");
            return false;
        }
    ...
    if (index_count && indices) {
        ...
        if(!upload_data_range(
            ...
            total_size, 
            indices)) {
                LERROR("vulkan_renderer_create_geometry failed to upload geometry data to the buffer!");
            }
    }
...
void vulkan_renderer_destroy_geometry(geometry* geometry) {
    if (geometry && geometry->internal_id != INVALID_ID) {
        ...
        // Clean up data.
        lzero_memory(internal_data, sizeof(vulkan_geometry_data));
