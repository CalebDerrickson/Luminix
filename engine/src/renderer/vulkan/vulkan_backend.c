...
b8 vulkan_renderer_backend_initilize(renderer_backend* backend, const char* application_name)
{
    ...
    // UI Renderpass creation
    LDEBUG("Creating ui renderpass...");
    vulkan_renderpass_create(
        &context,
...
void vulkan_renderer_backend_shutdown(renderer_backend* backend)
{
    ...
    LDEBUG("Destroying ui renderpass...");
    vulkan_renderpass_destroy(&context, &context.ui_renderpass);

    LDEBUG("Destroying world renderpass...");
...
b8 vulkan_renderer_backend_begin_frame(renderer_backend* backend, f32 delta_time)
{
    ...
    if(!vulkan_result_is_success(result)) {
        LERROR("In-flight fence wait failure! error: %s", vulkan_result_string(result, true));
        return false;
    }
...
void vulkan_renderer_update_global_world_state(
    mat4 projection, 
    mat4 view, 
    vec3 view_position, 
    vec4 ambient_color, 
...
void vulkan_renderer_update_global_ui_state(
    mat4 projection, 
    mat4 view, 
    i32 mode
)
