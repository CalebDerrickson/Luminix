...
// Internal functions for creation and destruction
...
)
{
    ...
    if (result == VK_ERROR_OUT_OF_DATE_KHR) {
        ...
        LFATAL("Failed to acquire swapchain image!");
        return false;
    }
...
void create(vulkan_context* context, u32 width, u32 height, vulkan_swapchain* swapchain)
{
    ...
    swapchain_extent.width = LCLAMP(swapchain_extent.width, min.width, max.width);
    ...
    // Instead of Hard-coding the max frames in flight to assume double triple buffering,
    // it should be based on the number of images. 
    ...
    LINFO("Swap chain created successfully.");
}
