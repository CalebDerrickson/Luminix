...
void renderer_on_resized(u16 width, u16 height) {
    if (state_ptr) {
        ...
        LWARN("renderer backend does not exist to accept resize: %i %i", width, height);
    }
}
...
b8 renderer_draw_frame(render_packet* packet) {
    ...
    if (state_ptr->backend.begin_frame(&state_ptr->backend, packet->delta_time)) {
        ...
        state_ptr->backend.update_global_world_state(state_ptr->projection, state_ptr->view, vec3_set(0.0f), vec4_set(1.0f), 0);
        ...
        if (!result) {
            LERROR("renderer_end_frame failed. Application shutting down...");
            return false;
        }
    }
