...
b8 load_texture (const char* texture_name, texture* t)
{
    ...
    char full_fule_path[512];

    // TODO: try different extensions
    string_format(full_fule_path, format_str, texture_name, "png");
    ...
    if (data) {
        ...
        if (stbi_failure_reason()) {
            LWARN("load_texture() failed to load file: %s : %s", full_fule_path, stbi_failure_reason());
        }
...
b8 renderer_system_initialize(u64* memory_requirement, void* state, const char* application_name) 
{
    *memory_requirement = sizeof(renderer_system_state);    
    ...
    // Take a pointer to default textures to use in the backend
